...
class AddressView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        address_list = Address.objects.filter(user=self.request.user).order_by(
            '-created')[0:10]
...
def set_default_address(request, id):
    ...
    messages.success(request,
                     'Your default delivery address has been successfully changed !')
    return HttpResponseRedirect(url)
...
def delete_address(request, id):
    ...
    try:
        ...
        messages.warning(request,
                         'Cannot delete a default address. Change your default address '
